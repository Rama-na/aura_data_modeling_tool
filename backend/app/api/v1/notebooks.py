...
async def generate_notebooks(
    ...
    if from_iter is None:
        from_iter = await state.get_current_iter_idx(r, session_id)

    if from_iter < 0:
        raise HTTPException(400, "No iterations available — run schema design first")

    # Validate it's complete
    detail = await state.get_iteration_detail(r, session_id, from_iter)
...
@router.get("/status")
async def get_notebook_status(session_id: str, r=Depends(get_redis)):
    job = await state.get_notebook_job(r, session_id)
    if not job:
...
@router.get("/download")
async def download_notebooks_zip(session_id: str, r=Depends(get_redis)):
    meta = await state.get_session(r, session_id)
    if not meta:
        raise HTTPException(404, "Session not found")
...
@router.get("/manifest")
async def get_notebook_manifest(session_id: str, r=Depends(get_redis)):
    manifest = get_manifest(session_id)
    return ok(manifest)


@router.get("/{filename}")
async def get_notebook(session_id: str, filename: str, r=Depends(get_redis)):
